...
It might be possible to break the REX events viewer with importing some bad
data. If you find that the events page isn't working, you can import `[]` to
...
- `bookmarks_only`: Set to **true** or **false** to change the "⭐️ Only"
  checkbox
- `time_filter`: Set to **all** for all events, **ongoing** for events happening
  now, **not_ended** for events that are happening now or in the future, and
...
[rex.mit.edu](https://rex.mit.edu) or [the API documentation](/rex/api).
