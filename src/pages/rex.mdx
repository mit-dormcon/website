...
export const REXName = () => {
    const [rexData, setRexData] = useState(null);
    useEffect(() => {
        const fetchRexData = () => {
            fetch("https://rex.mit.edu/api.json")
                .then((result) => result.json())
                .then((data) => setRexData(data));
        };
        fetchRexData();
    }, []);
    if (!rexData) {
...
export const REXEventDates = () => {
    ...
    const start = Date.parse(rexData.start);
    const end = Date.parse(rexData.end);

    const inThePast = end < Date.now();
    ...
    return `${rexData.name} ${inThePast ? "ran" : "run"} from ${startDate} to ${endDate}, though some events ${inThePast ? "occured" : "will occur"} before and after these dates.`;
...
**<REXEventDates />** Next year's events will be available around August, so
stay tuned!
...
information on
[the MIT Guide to Residences](https://mitguidetoresidences.mit.edu/first-year-housing-process).
