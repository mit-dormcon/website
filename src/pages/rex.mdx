...
export const REXEventDates = () => {
    ...
    return `${rexData.name} ${inThePast ? "ran" : "runs"} from ${startDate} to ${endDate}, though some events ${inThePast ? "occured" : "will occur"} before and after these dates.`;
...
physical REX booklets with the events will also be available.
